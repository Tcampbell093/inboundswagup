...
function renderQueue(){
  ...
  if(!readyFiltered.length){
    ...
    queueTableBody.innerHTML=readyVisible.map(row=>{const link=buildSalesforcePbLink(row.pbId,row.pdfUrl);const revenue=getQueueRowRevenue(row);return `<tr><td>${row.priority?'⭐':'—'}</td><td>${renderPbLink(row.pb||'—', row.pbId, row.pdfUrl)}</td><td>${escapeHtml(row.so||'—')}</td><td>${escapeHtml(row.account||'—')}</td><td>${Number(row.qty||0)}</td><td>${Number(row.products||0)}</td><td>${Number(row.units||0).toLocaleString()}</td><td>${escapeHtml(getEffectiveIhdForRow(row)||'—')}</td><td>${formatQueueMoney(revenue,{blankZero:true})}</td><td>${escapeHtml(row.status||'—')}</td><td>${renderQueueFlags(row)}</td><td>${escapeHtml(row.accountOwner||'—')}</td><td>${link?`<a class="queue-link" href="${escapeHtml(link)}" target="_blank" rel="noopener noreferrer">Open</a>`:'—'}</td><td><div class="row-actions"><button class="btn secondary" onclick="toggleQueuePriority('${escapeJs(String(row.id))}','ready')">${row.priority?'Unmark':'Priority'}</button><button class="btn warn" onclick="openIssueHoldModal('${escapeJs(String(row.id))}','ready')">Hold</button><button class="btn" onclick="scheduleQueueRow('${escapeJs(String(row.id))}','ready')">Schedule</button></div></td></tr>`}).join('');
  }
  ...
  scheduledQueueTableBody.innerHTML=scheduledSorted.length?scheduledVisible.map(row=>{const link=buildSalesforcePbLink(row.pbId,row.pdfUrl);const revenue=getQueueRowRevenue(row);return `<tr data-cbkey="${escapeHtml(row.pbId||row.so||'')}"><td>${renderPbLink(row.pb||'—', row.pbId, row.pdfUrl)}</td><td>${escapeHtml(row.so||'—')}</td><td>${escapeHtml(row.account||'—')}</td><td>${Number(row.units||0).toLocaleString()}</td><td>${escapeHtml(row.scheduledFor||'—')}</td><td>${escapeHtml(row.scheduledAt||'—')}</td><td>${formatQueueMoney(revenue,{blankZero:true})}</td><td>${renderQueueFlags(row)}</td><td>${escapeHtml(row.scheduleNote||'—')}</td><td>${link?`<a class="queue-link" href="${escapeHtml(link)}" target="_blank" rel="noopener noreferrer">Open</a>`:'—'}</td><td class="cb-cell" data-cbkey="${escapeHtml(row.pbId||row.so||'')}"><span class="cb-badge cb-loading">…</span></td><td><div class="row-actions"><button class="btn secondary" onclick="viewScheduledInAssembly('${escapeJs(String(row.id))}')">View in Assembly</button><button class="btn warn" onclick="openIssueHoldModal('${escapeJs(String(row.id))}','scheduled')">Hold</button><button class="btn secondary" onclick="unscheduleQueueRow('${escapeJs(String(row.id))}')">Unschedule</button><button class="btn danger" onclick="deleteScheduledQueueRow('${escapeJs(String(row.id))}')">Delete</button></div></td></tr>`}).join(''):'<tr><td colspan="12" class="empty">Nothing has been scheduled from the queue yet.</td></tr>';
  ...
  return `<div class="mob-card">
    <div class="mob-card-header">
      <div class="mob-card-title">
        <span class="mob-card-pb">${renderPbLink(row.pb||'—', row.pbId, row.pdfUrl)}${row.priority?' ⭐':''}</span>
...
function buildScheduledQueueCard(row) {
  ...
  return `<div class="mob-card" data-cbkey="${cbKey}">
    <div class="mob-card-header">
      <div class="mob-card-title">
        <span class="mob-card-pb">${renderPbLink(row.pb||'—', row.pbId, row.pdfUrl)}</span>
        <span class="mob-card-account">${escapeHtml(row.account||'—')}</span>
      </div>

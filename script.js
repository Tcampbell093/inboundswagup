...
function setAssemblyDateAndNavigate(dateStr,openAssemblyPage=true){
  ...
  if(openAssemblyPage){
    ...
    const assemblyNavBtn=document.querySelector('[data-page="assemblyPage"]');
    if(assemblyNavBtn) assemblyNavBtn.classList.add('active');
    const assemblyPage=document.getElementById('assemblyPage');
    if(assemblyPage) assemblyPage.classList.add('active');
  }
  renderCalendar();
  renderAssembly();
  setTimeout(()=>{
...
function viewScheduledInAssembly(id){
  ...
  navButtons.forEach(b=>b.classList.remove('active'));
  const assemblyNav=[...navButtons].find(btn=>btn.dataset.page==='assemblyPage');
  if(assemblyNav) assemblyNav.classList.add('active');
  pages.forEach(p=>p.classList.remove('active'));
  document.getElementById('assemblyPage').classList.add('active');

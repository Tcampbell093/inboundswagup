...
const assemblySyncKeys=new Set([assemblyBoardStorageKey,queueStorageKey,scheduledQueueStorageKey,incompleteQueueStorageKey,revenueReferenceStorageKey,calendarEventsStorageKey,issueHoldQueueStorageKey,'ops_hub_queue_import_meta_v1','ops_hub_revenue_import_meta_v1']);
...
function addErrorRecord(){const metrics=calculateErrorMetrics();const record={id:Date.now(),date:errorDateInput.value,department:errorDepartmentInput.value,associate:errorAssociateInput.value.trim(),proofed:errorProofedInput.value,poNumber:errorPoNumberInput.value.trim(),linkedId:errorLinkedIdInput.value.trim(),category:errorCategoryInput.value.trim(),palletLocation:errorPalletLocationInput.value.trim(),expectedQty:Number(errorExpectedQtyInput.value||0),receivedQty:Number(errorReceivedQtyInput.value||0),errorType:errorTypeInput.value,absoluteAmount:metrics.absolute,errorRate:metrics.rate,notes:errorNotesInput.value.trim()};if(!record.associate){alert('Add the associate name first.');errorAssociateInput.focus();return}errorRecords.unshift(record);if(typeof logHistory==='function')logHistory({entity_type:'error_record',entity_id:record.poNumber||String(record.id),action:'created',after_data:{date:record.date,associate:record.associate,department:record.department,errorType:record.errorType,errorRate:record.errorRate,poNumber:record.poNumber},related_type:record.associate?'employee':null,related_id:record.associate||null,note:record.notes||null});saveJson(errorsStorageKey,errorRecords);clearErrorForm();renderErrors()}
function deleteErrorRecord(id){const _er=errorRecords.find(r=>r.id===id);if(typeof logHistory==='function'&&_er)logHistory({entity_type:'error_record',entity_id:_er.poNumber||String(_er.id),action:'deleted',before_data:{date:_er.date,associate:_er.associate,department:_er.department,errorType:_er.errorType,errorRate:_er.errorRate,poNumber:_er.poNumber},related_type:_er.associate?'employee':null,related_id:_er.associate||null});errorRecords=errorRecords.filter(r=>r.id!==id);saveJson(errorsStorageKey,errorRecords);renderErrors()}

...
function renderQueue(){
  ...
  <td><div class="row-actions">${link?`<button class="btn secondary" onclick="window.open('${escapeHtml(link)}','_blank')">Open</button>`:'<button class="btn secondary" disabled>Open</button>'}<button class="btn secondary" onclick="viewScheduledInAssembly('${escapeJs(String(row.id))}')">View in Assembly</button><button class="btn secondary" onclick="unscheduleQueueRow('${escapeJs(String(row.id))}')">Unschedule</button><button class="btn danger" onclick="deleteScheduledQueueRow('${escapeJs(String(row.id))}')">Delete</button></div></td></tr>`'<tr><td colspan="10" class="empty">Nothing has been scheduled from the queue yet.</td></tr>';
}

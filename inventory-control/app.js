/* =========================================================
   app.js — Standalone Warehouse Hub Inventory
   ---------------------------------------------------------
...
(function () {
  ...
  var API = '/.netlify/functions/warehouse-inventory';
  ...
  function loadData() {
    ...
    ivFetch(API + (state.includeArchived ? '?includeArchived=1' : ''), { headers: { 'Accept': 'application/json' } })
  ...
  function post(payload, done) {
    ivFetch(API, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify(payload) })
      .then(function (r) { return r.json().then(function (j) { return { ok: r.ok, j: j }; }); })
  ...
  function loadEmailLog() {
    ...
    if (!box) return;
    ivFetch(API + '?emailLog=1', { headers: { 'Accept': 'application/json' } })
      .then(function (r) { return r.json().then(function (j) { return { ok: r.ok, j: j }; }); })
      .then(function (res) {

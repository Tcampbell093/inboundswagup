...
function normalizeLocalCleaning(row) {
  let status=row.status || 'scheduled'; const workDate=dateValue(row.work_date);
    ...
    const cleaning = (Array.isArray(data.assignments) ? data.assignments : [])
      ...
      .map(a => {
        ...
        return {
          ...
          source: 'fairshift',
          checkinUrl: `${FAIRSHIFT_BASE}/checkin?assignment=${encodeURIComponent(fairshiftId)}`,
...
async function upsertCleaning(body) {
  const date=validDate(body.date),area=text(body.area,100),employeeName=text(body.employeeName,100);
  if(!date||!area||!employeeName) throw new Error('Date, cleaning area, and employee are required.');
  const id=text(body.id,80)||crypto.randomUUID();
  await pool.query(`INSERT INTO hub_cleaning_assignments(id,work_date,area,task,employee_name,status,source,updated_at) VALUES($1,$2,$3,$4,$5,'scheduled','hub',NOW()) ON CONFLICT(id) DO UPDATE SET work_date=EXCLUDED.work_date,area=EXCLUDED.area,task=EXCLUDED.task,employee_name=EXCLUDED.employee_name,updated_at=NOW()`,[id,date,area,text(body.task,240),employeeName]);
  return {id};
}
async function upsertEmployee(body) {
  const name=text(body.name,100),pin=text(body.pin,12),department=text(body.department,100);
  if(!name) throw new Error('Employee name is required.');
  if(!/^\d{4,8}$/.test(pin)) throw new Error('PIN must be 4–8 digits.');
  if(!process.env.HUB_PIN_SALT) throw new Error('HUB_PIN_SALT is not configured.');
  const key=slug(name);
  await pool.query(`INSERT INTO hub_employee_pins(employee_key,employee_name,department,pin_hash,active,updated_at) VALUES($1,$2,$3,$4,TRUE,NOW()) ON CONFLICT(employee_key) DO UPDATE SET employee_name=EXCLUDED.employee_name,department=EXCLUDED.department,pin_hash=EXCLUDED.pin_hash,active=TRUE,updated_at=NOW()`,[key,name,department,hashPin(pin)]);
...
async function cleaningAction(body) {
  ...
  const employee=await pool.query(`SELECT employee_name,pin_hash,active FROM hub_employee_pins WHERE employee_key=$1 LIMIT 1`,[slug(employeeName)]); const e=employee.rows[0];
  if(!e||!e.active||!safeEqual(e.pin_hash,hashPin(pin))) throw new Error('Name or PIN is incorrect.');
  const assignment=await pool.query(`SELECT * FROM hub_cleaning_assignments WHERE id=$1 LIMIT 1`,[assignmentId]); const row=assignment.rows[0];
  if(!row) throw new Error('Cleaning assignment not found.');
  const workDate=dateValue(row.work_date); if(workDate!==todayEastern()) throw new Error('Cleaning can only be checked in for today.');
...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    else if(body.action==='upsertCleaning') result=await upsertCleaning(body);
    else if(body.action==='upsertEmployee') result=await upsertEmployee(body);
    ...
    else if(body.action==='deleteCleaning'){body.kind='cleaning';await deleteRecord(body);}
    else if(body.action==='deleteEmployee'){body.kind='employee';await deleteRecord(body);}
    else return json(400,{error:'Unsupported action.'});
    return json(200,{ok:true,result});
  } catch(error) { return json(400,{error:text(error?.message||'Unexpected error.',300)}); }
};

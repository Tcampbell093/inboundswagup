...
function managerAuthorized(event) {
  const key = event.headers?.['x-hub-key'] || event.headers?.['X-Hub-Key'] || '';
  return safeEqual(key, process.env.HUB_MANAGER_KEY || '');
}

function slug(value) {
  return text(value, 100).toLowerCase().normalize('NFKD').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 80);
}

function hashPin(pin) {
  return crypto.createHash('sha256').update(`${process.env.HUB_PIN_SALT || ''}:${pin}`).digest('hex');
}
...
async function ensureSchema() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS hub_announcements (
      ...
      end_date DATE,
      department TEXT NOT NULL DEFAULT 'All teams',
...
function normalizeCleaning(row) {
  ...
  return {
    ...
    creditMinutes: Number(row.credit_minutes || 0),
    source: row.source || 'hub',
    updatedAt: row.updated_at || null,
  };
}

async function readFeed(includeEmployees = false) {
  ...
  const result = {
    ...
    cleaning: c.rows.map(normalizeCleaning),
...
async function cleaningAction(body) {
  ...
  return normalizeCleaning(updated.rows[0]);
}

exports.handler = async function handler(event) {
  if (!process.env.DATABASE_URL) return json(500,{error:'DATABASE_URL is not configured'});
  if (!process.env.HUB_PIN_SALT) return json(500,{error:'HUB_PIN_SALT is not configured'});
  try {
    await ensureSchema();

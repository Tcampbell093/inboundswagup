/* =========================================================
   warehouse-inventory.js — Standalone Warehouse Hub Inventory API
   ---------------------------------------------------------
...
async function ensureSchema() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS hub_inventory_items (
  ...
  `);
  await pool.query(`ALTER TABLE hub_inventory_items ADD COLUMN IF NOT EXISTS order_link TEXT;`);
  await pool.query(`ALTER TABLE hub_inventory_items ADD COLUMN IF NOT EXISTS spot TEXT;`);
  ...
  await pool.query(`ALTER TABLE hub_inventory_items ADD COLUMN IF NOT EXISTS product_key TEXT;`);
  await pool.query(`CREATE INDEX IF NOT EXISTS hub_inventory_items_active_idx ON hub_inventory_items(archived);`);
  await pool.query(`CREATE INDEX IF NOT EXISTS hub_inventory_items_product_key_idx ON hub_inventory_items(product_key);`);
  await pool.query(`
    CREATE TABLE IF NOT EXISTS hub_inventory_requests (
      id            BIGSERIAL PRIMARY KEY,
      item_id       BIGINT,
      item_name     TEXT NOT NULL,
      category      TEXT,
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS hub_inventory_requests_status_idx ON hub_inventory_requests(status);`);
  await pool.query(`
    CREATE TABLE IF NOT EXISTS hub_inventory_photos (
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS hub_inventory_subscriptions (
      id                BIGSERIAL PRIMARY KEY,
      email             TEXT NOT NULL,
      label             TEXT,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS hub_inventory_meta (
      key TEXT PRIMARY KEY,
      value TEXT NOT NULL,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );
  `);
  const snapshotMarker = await pool.query(`SELECT value FROM hub_inventory_meta WHERE key='houston_snapshot_v1' LIMIT 1;`);
  if (!snapshotMarker.rows.length) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      await client.query(`
        INSERT INTO hub_inventory_items
          (id,item_name,category,department,location,spot,quantity,unit_type,min_stock,vendor,sku,order_link,notes,needs_review,archived,last_counted,last_updated_by,created_by,created_at,updated_at,product_key)
        SELECT
          id,item_name,category,department,location,spot,quantity,unit_type,min_stock,vendor,sku,order_link,notes,needs_review,archived,last_counted,last_updated_by,created_by,created_at,updated_at,product_key
      ...
      `);
      await client.query(`
        INSERT INTO hub_inventory_requests
          (id,item_id,item_name,category,department,quantity,urgency,reason,status,requested_by,assigned_to,expected_date,tracking,order_link,notes,created_at,updated_at)
        SELECT
          id,item_id,item_name,category,department,quantity,urgency,reason,status,requested_by,assigned_to,expected_date,tracking,order_link,notes,created_at,updated_at
      ...
      `);
      await client.query(`
        INSERT INTO hub_inventory_photos(item_id,photo_data,taken_by,taken_at)
        SELECT item_id,photo_data,taken_by,taken_at
        FROM inventory_photos
        ON CONFLICT(item_id) DO NOTHING;
      `);
      await client.query(`
        INSERT INTO hub_inventory_subscriptions(id,email,label,enabled,unsubscribe_token,created_by,created_at)
        SELECT id,email,label,enabled,unsubscribe_token,created_by,created_at
        FROM inventory_subscriptions
        ON CONFLICT(id) DO NOTHING;
      `);
      await client.query(`
        INSERT INTO hub_inventory_request_emails(id,request_id,email,status,error,sent_at)
        SELECT id,request_id,email,status,error,sent_at
        FROM inventory_request_emails
        ON CONFLICT(id) DO NOTHING;
      `);

      await client.query(`SELECT setval(pg_get_serial_sequence('hub_inventory_items','id'), GREATEST(COALESCE((SELECT MAX(id) FROM hub_inventory_items),0),1), EXISTS(SELECT 1 FROM hub_inventory_items));`);
      await client.query(`SELECT setval(pg_get_serial_sequence('hub_inventory_requests','id'), GREATEST(COALESCE((SELECT MAX(id) FROM hub_inventory_requests),0),1), EXISTS(SELECT 1 FROM hub_inventory_requests));`);
      await client.query(`SELECT setval(pg_get_serial_sequence('hub_inventory_subscriptions','id'), GREATEST(COALESCE((SELECT MAX(id) FROM hub_inventory_subscriptions),0),1), EXISTS(SELECT 1 FROM hub_inventory_subscriptions));`);
      await client.query(`SELECT setval(pg_get_serial_sequence('hub_inventory_request_emails','id'), GREATEST(COALESCE((SELECT MAX(id) FROM hub_inventory_request_emails),0),1), EXISTS(SELECT 1 FROM hub_inventory_request_emails));`);

      const counts = await client.query(`
        SELECT
          (SELECT COUNT(*)::int FROM hub_inventory_items) AS items,
          (SELECT COUNT(*)::int FROM hub_inventory_requests) AS requests,
          (SELECT COUNT(*)::int FROM hub_inventory_photos) AS photos,
          (SELECT COUNT(*)::int FROM hub_inventory_subscriptions) AS subscriptions,
          (SELECT COUNT(*)::int FROM hub_inventory_request_emails) AS email_logs;
      `);
      await client.query(
        `INSERT INTO hub_inventory_meta(key,value,updated_at) VALUES('houston_snapshot_v1',$1,NOW()) ON CONFLICT(key) DO NOTHING;`,
        [JSON.stringify({ copiedAt: new Date().toISOString(), ...counts.rows[0] })],
      );
  ...
  const subCount = await pool.query(`SELECT COUNT(*)::int AS n FROM hub_inventory_subscriptions;`);
  if (subCount.rows[0].n === 0) {
    await pool.query(
      `INSERT INTO hub_inventory_subscriptions (email, label, unsubscribe_token, created_by)
...
const NOTIFY_FROM = process.env.INVENTORY_NOTIFY_FROM || process.env.FROM_EMAIL || 'Warehouse Inventory <onboarding@resend.dev>';
const GMAIL_USER = (process.env.GMAIL_USER || '').trim();
// App passwords are often shown with spaces ("abcd efgh ijkl mnop") — strip them.
const GMAIL_APP_PASSWORD = (process.env.GMAIL_APP_PASSWORD || '').replace(/\s+/g, '');
const GMAIL_FROM = process.env.GMAIL_FROM || (GMAIL_USER ? `Warehouse Inventory <${GMAIL_USER}>` : '');
...
function buildRequestEmail(req, token) {
  ...
  const orderBtn = req.orderLink
    ...
    : '';
  const appBtn = `<a href="${htmlEscape(SITE_URL)}/inventory-control/" style="display:inline-block;margin:4px 8px 4px 0;padding:9px 16px;background:#11324f;color:#fff;border-radius:6px;text-decoration:none;font-size:14px;font-weight:600">View in Command Tool</a>`;
  const unsub = token
    ? `<p style="margin:18px 0 0;font-size:11px;color:#9aa7b4">You're receiving this because you subscribed to inventory request alerts. <a href="${htmlEscape(SITE_URL)}/.netlify/functions/warehouse-inventory?unsub=${encodeURIComponent(token)}" style="color:#9aa7b4">Unsubscribe</a>.</p>`
    : '';
...
async function notifyNewRequest(req, caller) {
  ...
  try {
    ...
    subs = (await pool.query(`SELECT email, unsubscribe_token FROM hub_inventory_subscriptions WHERE enabled=true;`)).rows;
    for (const s of subs) {
      ...
      try {
        ...
        try {
          await pool.query(
            `INSERT INTO hub_inventory_request_emails (request_id, email, status, error) VALUES ($1,$2,$3,$4);`,
...
exports.handler = async function handler(event) {
  ...
  if (event.httpMethod === 'GET' && (qs.snapshotStatus === '1' || qs.snapshotStatus === 'true')) {
    ...
    const counts = await pool.query(`
      SELECT
        ...
        (SELECT COUNT(*)::int FROM hub_inventory_request_emails) AS email_logs;
    `);
    return json(200, { ok: true, independent: true, snapshot: marker.rows[0] || null, counts: counts.rows[0] || {} });
  }
  ...
  if (event.httpMethod === 'GET' && qs.photo && (qs.img === '1' || qs.img === 'true')) {
    try {
      await ensureSchema();
      const r = await pool.query('SELECT photo_data FROM hub_inventory_photos WHERE item_id=$1;', [qs.photo]);
  ...
  if (event.httpMethod === 'GET' && qs.unsub) {
    ...
    try {
      await ensureSchema();
      const r = await pool.query(`UPDATE hub_inventory_subscriptions SET enabled=false WHERE unsubscribe_token=$1 RETURNING email;`, [String(qs.unsub)]);
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      if (qs.emailLog === '1' || qs.emailLog === 'true') {
        ...
        const lr = await pool.query(
          `SELECT e.email, e.status, e.error, e.sent_at, r.item_name
             ...
             LEFT JOIN hub_inventory_requests r ON r.id = e.request_id
      ...
      const r = await pool.query(
        `SELECT * FROM hub_inventory_items ${includeArchived ? '' : 'WHERE archived=false'} ORDER BY item_name ASC;`
      );
      const items = r.rows.map(rowToItem);
      // Which items have a photo (+ a version stamp for cache-busting the <img>).
      const ph = await pool.query(`SELECT item_id, EXTRACT(EPOCH FROM taken_at)*1000 AS ver FROM hub_inventory_photos;`);
      ...
      const rq = await pool.query(`SELECT status, urgency FROM hub_inventory_requests;`);
      ...
      const rpm = await pool.query(`SELECT item_id, COUNT(*)::int AS n FROM hub_inventory_requests WHERE item_id IS NOT NULL AND created_at >= NOW() - INTERVAL '30 days' GROUP BY item_id;`);
    ...
    if (action === 'add') {
      ...
      if (!body.force) {
        const dupQ = await pool.query(
          `SELECT * FROM hub_inventory_items WHERE archived=false
      ...
      if (body.linkTo) {
        const lk = await pool.query(`SELECT product_key FROM hub_inventory_items WHERE id=$1 LIMIT 1;`, [body.linkTo]);
        if (lk.rows.length) {
          const key = lk.rows[0].product_key || ('pk_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 8));
          await pool.query(`UPDATE hub_inventory_items SET product_key=$1, updated_at=NOW() WHERE id = ANY($2::bigint[]);`, [key, [newItem.id, body.linkTo]]);
    ...
    if (action === 'count') {
      ...
      const cur = await pool.query(`SELECT * FROM hub_inventory_items WHERE id=$1;`, [id]);
      ...
      const r = await pool.query(
        `UPDATE hub_inventory_items SET quantity=$1, needs_review=false, notes=$2, last_counted=NOW(),
    ...
    if (action === 'review') {
      if (!canCount) return json(403, { error: 'Not permitted' });
      const r = await pool.query(
        `UPDATE hub_inventory_items SET needs_review=false, last_updated_by=$1, updated_at=NOW() WHERE id=$2 RETURNING *;`,
        [who(caller), body.id]
      );
      if (!r.rows.length) return json(404, { error: 'not found' });
      return json(200, { ok: true, item: rowToItem(r.rows[0]) });
    ...
    if (action === 'delete') {
      ...
      await pool.query(`DELETE FROM hub_inventory_items WHERE id=$1;`, [body.id]);
    ...
    if (action === 'link') {
      ...
      const cur = await pool.query(
        `SELECT product_key FROM hub_inventory_items WHERE id = ANY($1::bigint[]) AND product_key IS NOT NULL LIMIT 1;`, [ids]
      );
      const key = (cur.rows[0] && cur.rows[0].product_key) ||
        ('pk_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 8));
      await pool.query(
        `UPDATE hub_inventory_items SET product_key=$1, last_updated_by=$2, updated_at=NOW() WHERE id = ANY($3::bigint[]);`,
        [key, who(caller), ids]
      );
    ...
    if (action === 'unlink') {
      ...
      await pool.query(`UPDATE hub_inventory_items SET product_key=NULL, last_updated_by=$1, updated_at=NOW() WHERE id=$2;`, [who(caller), body.id]);
      // A group of one is meaningless — if a lone row is left on that key, clear it too.
      if (key) {
        await pool.query(
          `UPDATE hub_inventory_items SET product_key=NULL
             WHERE product_key=$1 AND (SELECT COUNT(*) FROM hub_inventory_items WHERE product_key=$1) = 1;`,
          [key]
        );
      }
      return json(200, { ok: true });
    ...
    if (action === 'import') {
      ...
      for (const row of rows) {
        ...
        if (id) {
          await pool.query(
            `UPDATE hub_inventory_items SET
    ...
    if (action === 'subAdd') {
      ...
      const r = await pool.query(
        `INSERT INTO hub_inventory_subscriptions (email, label, unsubscribe_token, created_by)
         ...
         ON CONFLICT (LOWER(email)) DO UPDATE SET enabled=true, label=COALESCE(NULLIF(EXCLUDED.label,''), hub_inventory_subscriptions.label)
    ...
    if (action === 'removePhoto') {
      ...
      await pool.query(`DELETE FROM hub_inventory_photos WHERE item_id=$1;`, [body.itemId]);
      return json(200, { ok: true });
    }

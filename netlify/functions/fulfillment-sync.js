...
async function ensureSchema() {
  ...
  `);
  await pool.query(`
    INSERT INTO fulfillment_sync_state (state_key, state_json)
...
exports.handler = async function handler(event) {
  const _g = await require('./_auth').guard(event, 'fulfillment-sync');
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      const result = await pool.query(
        "SELECT state_json, updated_at FROM fulfillment_sync_state WHERE state_key='default' LIMIT 1;"
      );
      const row = result.rows[0] || { state_json: {}, updated_at: null };
    ...
    if (event.httpMethod === 'POST') {
      ...
      const result = await pool.query(
        "INSERT INTO fulfillment_sync_state (state_key, state_json, updated_at) " +
    ...
    return json(500, { error: error.message || 'Unknown fulfillment sync error' });
  }
};

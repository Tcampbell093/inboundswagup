...
function renderAssemblyFocusList(sortedWithPriority){
  ...
  list.innerHTML=filtered.map(function({row,priority}){
    ...
    return '<div class="asm-focus-row" data-cbkey="'+escapeHtml(cbKey)+'" data-asm-id="'+row.id+'" style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:10px;border:1px solid var(--blue2);border-left:3px solid '+borderColor+';background:var(--card);opacity:'+(isDone?'0.55':'1')+';">' +
      (isPackBuilderWorkType(row.workType) ? '<input type="checkbox" class="asm-bulk-cb" data-asm-id="'+row.id+'" aria-label="Select row" style="flex-shrink:0;width:16px;height:16px;cursor:pointer;" />' : '<span style="width:16px;flex-shrink:0;"></span>') +
      ...
      '<div style="flex:1;min-width:0;">' +
        '<div style="display:flex;align-items:center;gap:6px;margin-bottom:2px;">' +
          '<span style="font-weight:700;font-size:13px;">'+renderPbLink(row.pb||row.so||'—', row.pbId, row.pdfUrl, {cls:'pb-link pb-link-strong'})+'</span>' +
      ...
      '<div style="display:flex;align-items:center;gap:6px;flex-shrink:0;">' +
        ...
        (isPackBuilderWorkType(row.workType)?'<button class="btn secondary" onclick="rescheduleAssemblyBoardRow('+row.id+')" style="font-size:12px;padding:4px 10px;">Reschedule</button>':'') +
        (isPackBuilderWorkType(row.workType)?'<button class="btn warn" onclick="removeAssemblyBoardRow('+row.id+')" style="font-size:12px;padding:4px 10px;">Unschedule</button>':'') +
